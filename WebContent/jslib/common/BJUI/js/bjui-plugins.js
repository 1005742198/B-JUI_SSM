...
+function ($) {
    ...
    $(document).on(BJUI.eventType.initUI, function(e) {
        ...
        // UI init begin...
//						        //modified by hj
//						        var unReadCount=$box.find('#unReadCount');
//						        if(unReadCount.text()==''){
//						        	unReadCount.hide();
//						        }else{
//						        	unReadCount.show();
...
//								            })
	    ...
	    if($selectpicker){
	        ...
	        $selectpicker.each(function() {
	            ...
	            //modified by hj
	//			setTimeout(function(){
	//				var $scroll = $element.parents().filter(function() {
	//					if($element.parents().is('.bjui-dialog')) {
	//						return true;
	//					}else{
	//						return this.scrollHeight>$(this).innerHeight();
	//					}
	//				});
	//				$element.selectpicker('selectPosition',$scroll);
	//			},1000);
        ...
        if($textOmit){
	        $textOmit.each(function(){
	        	...
	        	var _resize=function(){
		        	...
		        	if($textTit.length){
		        		var width=$(_this).parents('td').width()-$textTit.outerWidth();
		        	}else{
		        		var width=$(_this).parents('td').width()
		        	}
